# BTC Analytics V2 — P0 Canonical v6
...
Aucun code de production ne doit être écrit avant validation explicite de P0. La révision candidate v6 reste en attente de validation utilisateur.
...
| D-020 | Every canonical closed candle has an append-only revision lineage: first valid ingestion creates `revision_seq = 1` as `accepted_current` with `observed_at` and `accepted_at`; each later distinct observation allocates the next `revision_seq` at candidate creation, even if quarantined. Changed accepted candles use revision-acceptance policy v1: validate, confirm against the same venue's native authoritative endpoint, promote only on agreement, otherwise quarantine. Accepted revisions satisfy `observed_at <= accepted_at`; `observed_point_in_time` selects only the latest accepted revision with `accepted_at <= T`; quarantined revisions are never PIT-eligible; snapshots never retroactively claim a late correction was observed or accepted at historical T. | RECOMMENDED — AWAITING USER VALIDATION |
